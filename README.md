...
- If a decorated task function's argument list includes `log`, `iotaa` will intercept the value of a `log` **keyword** argument (which should be a Python `Logger` object) passed to it, and use that logger for all informational messages logged automatically by `iotaa` on behalf of the task. The task may also log its own messages via `log.info()`, `log.debug()`, etc., so that messages from `iotaa` and from user code appear in a single, coherent stream.
